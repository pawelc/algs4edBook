...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
 ...
 // add the keys between lo and hi in the subtree rooted at x
    // to the queue
    private void redKeys(Node x, Queue<Key> queue) { 
        if (x == null) return; 
        if(isRed(x)) queue.enqueue(x.key); 
        redKeys(x.left, queue); 
        redKeys(x.right, queue); 
    } 
    
    public Iterable<Key> redKeys(){
        Queue<Key> reds = new Queue<Key>();
        redKeys(root, reds);
        return reds;
    }
    
    public Iterable<Key> keysLevelOrder(){
        Queue<Node> toProcess = new Queue<Node>();
        Queue<Key> level = new Queue<Key>();
        toProcess.enqueue(root);
        while(!toProcess.isEmpty()){
            Node current = toProcess.dequeue();
            level.enqueue(current.key);
            if(current.left != null) toProcess.enqueue(current.left);
            if(current.right != null) toProcess.enqueue(current.right);
        }
        return level;
    }
